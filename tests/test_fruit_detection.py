...
from fruitripeness.ui_core import comparison_sheet
...
class FruitDetectionTests(unittest.TestCase):
    ...
    def test_each_detected_fruit_is_classified_independently(self):
        ...
        card = per_fruit_card(row, analysis)
        self.assertGreater(card.height, 500)
        sheet = comparison_sheet("two-fruits.png", [card])
        self.assertEqual(sheet.size, (card.width + 20, card.height + 110))
        self.assertEqual(
            sheet.getpixel((10, 80 + card.height - 1)),
